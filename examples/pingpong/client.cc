 ...
 *
 *        Version:  0.0.1
 *        Created:  11/21/2012 04:19:48 PM
 *       Revision:  r1
	...
	public :
		Client(EventLoop* loop,
			...
			: loop_(loop),
			  ...
			  timeout_(timeout)
		{
			loop_->runAfter(timeout, boost::bind(&Client::handleTimeout, this));
			if(threadCount > 1)
			{
		...
		void onDisconnect()
		{
			if(numConnected_.decrementAndGet() == 0)
			{
						...
						it != sessions_.end(); it++)
				{
					...
					totalMessageRead += it->messagesRead();
				}
				LOG_WARN << totalBytesRead << " total bytes read";
				LOG_WARN << totalMessageRead << " total messages read";
				LOG_WARN << static_cast<double>(totalBytesRead)/static_cast<double>(totalMessageRead) << " average message size";
...
int main(int argc, char* argv[])
{
	if(argc != 7)
	{
		fprintf(stderr, "Usage: client <host_pi> <port> <threads> <blocksize> ");
		fprintf(stderr, "<session> <time>\n");
	}
	else 
	{
